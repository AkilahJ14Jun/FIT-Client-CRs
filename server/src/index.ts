...
app.post("/api/whatsapp/send-direct", async (req, res) => {
  ...
  try {
    ...
    const uploadRes = await fetch(`https://${apiHost}/wapp/api/upload/media`, {
      method: 'POST',
      headers: {
        'X-API-KEY': apiKey
      },
    ...
    const mediaid = uploadData.mediaid || uploadData.id || uploadData.mediaId || uploadData.data?.mediaid || uploadData.data?.id;
    ...
    const sendRes = await fetch(`https://${apiHost}/wapp/v2/api/send`, {
      method: 'POST',
      body: sendFormData
    });
    ...
    try {
      ...
      if (fs.existsSync(envPath)) {
        ...
        if (!foundRemaining) lines.push(`Remaining Msg Count=4999`);
        if (!foundTotal) lines.push(`Total Msg Sent=1`);
        if (!foundToday) lines.push(`Msg Sent Today=1`);
        if (!foundLastDate) lines.push(`Last Sent Date=${todayStr}`);
